...
trait GameDef {
  ...
  def startBlock: Block = {
    val test = Block(startPos,startPos)
    test}
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def neighbors: List[(Block, Move)] = {
            	  val test = List((this.left,Left),(this.right,Right),(this.down,Down),(this.up,Up))
//                  println("neighbors" + test)
            	  test
               }
     ...
     */
    def legalNeighbors: List[(Block, Move)] =  {val test = this.neighbors filter (tuple => tuple._1.isLegal)
    				
//    			println("legalNeighbors" + test)
    			test
    			}
