...
class Tweet(val user: String, val text: String, val retweets: Int) {
  ...
  def containsKeyword(k: String): Boolean =
...
abstract class TweetSet {
  ...
  def print : Unit =foreach( (f : Tweet) => println(f.text))
}
...
class Empty extends TweetSet {
  ...
  override def descendingByRetweetAcc(acc: TweetList) = acc 
  /**
   * The following methods are already implemented
   */
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweetedAcc(acc: Tweet)= (left union right).mostRetweetedAcc( if (acc.retweets > elem.retweets) acc else elem) 
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(t  => t.containsAnyKeywords(google))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(t  => t.containsAnyKeywords(apple))
